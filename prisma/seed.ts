...
async function main() {
    ...
    // 11. Buyer Contacts
    const buyerContacts = [
        { id: '88888888-8888-4888-8888-666666666666', workspaceId: bkkRealty.id, contactType: ['Buyer'], firstName: 'Nattapong', lastName: 'Wongsakul', nickname: 'เบนซ์', phonePrimary: '085-999-1234', email: 'nattapong.w@gmail.com', lineId: 'nattapong_benz', nationality: 'Thai', contactSource: ContactSource.LINE, contactStatus: ContactStatus.ACTIVE, potentialTier: 'A' as const, assignedToId: users[2].id, createdById: users[0].id },
        { id: '88888888-8888-4888-8888-777777777777', workspaceId: bkkRealty.id, contactType: ['Buyer'], firstName: 'Sarah', lastName: 'Miller', nickname: null, phonePrimary: '092-555-6677', email: 'sarah.miller@company.com', lineId: 'sarah_m_bkk', nationality: 'American', contactSource: ContactSource.WEBSITE, contactStatus: ContactStatus.ACTIVE, potentialTier: 'B' as const, assignedToId: users[0].id, createdById: users[0].id },
        { id: '88888888-8888-4888-8888-888888888888', workspaceId: bkkRealty.id, contactType: ['Buyer'], firstName: 'Pimchanok', lastName: 'Luevisadpaibul', nickname: 'ปิ่น', phonePrimary: '063-888-9900', email: 'pin.pim@hotmail.com', lineId: null, nationality: 'Thai', contactSource: ContactSource.REFERRAL, contactStatus: ContactStatus.ACTIVE, potentialTier: 'A' as const, assignedToId: users[2].id, createdById: users[1].id, referredById: contacts[2].id },
        { id: '88888888-8888-4888-8888-999999999999', workspaceId: bkkRealty.id, contactType: ['Buyer', 'Seller'], firstName: 'Kenji', lastName: 'Tanaka', nickname: null, phonePrimary: '094-222-3344', email: 'kenji.t@outlook.jp', lineId: 'kenji_bkk', nationality: 'Japanese', contactSource: ContactSource.FACEBOOK, contactStatus: ContactStatus.ACTIVE, potentialTier: 'C' as const, assignedToId: users[3].id, createdById: users[0].id },
        { id: '88888888-8888-4888-8888-aaaaaaaaaaaa', workspaceId: bkkRealty.id, contactType: ['Buyer'], firstName: 'Waraporn', lastName: 'Jantarasiri', nickname: 'หวาน', phonePrimary: '097-333-4455', contactSource: ContactSource.COLD_CALL, contactStatus: ContactStatus.ACTIVE, potentialTier: 'D' as const, assignedToId: users[3].id, createdById: users[1].id },
    ]
    ...
    for (const bc of buyerContacts) {
        await prisma.contact.upsert({
            ...
            create: {
                ...
                nickname: bc.nickname ?? null,
                phonePrimary: bc.phonePrimary,
                email: bc.email ?? null,
                lineId: bc.lineId ?? null,
                nationality: bc.nationality ?? null,
                contactSource: bc.contactSource,
                contactStatus: bc.contactStatus,
                potentialTier: bc.potentialTier,
                assignedToId: bc.assignedToId,
                createdById: bc.createdById,
                referredById: (bc as any).referredById ?? null,
            },
        })
    }

    // 12. Deals (mix of buy-side and sell-side, various stages)
    const deals = [
        // Buy-side deals
        {
            id: 'dddddddd-dddd-4ddd-8ddd-111111111111',
            workspaceId: bkkRealty.id,
            dealName: 'Nattapong — Sukhumvit Condo — Buy',
            dealType: DealType.BUY_SIDE,
            buyerContactId: buyerContacts[0].id,
            listingId: null,
            ...
            commissionRate: 3,
            estimatedCommission: 180000,
            assignedToId: users[2].id,
            createdById: users[0].id,
            notes: 'Looking for 1BR condo near BTS Siam or Asoke. Budget up to 6M. Very motivated buyer.',
            // Buyer requirements
            budgetMin: 4000000,
            budgetMax: 6500000,
            preferredZoneIds: [zones[0].id, zones[10].id], // Sukhumvit, Asoke
            preferredPropertyType: [PropertyType.CONDO],
            preferredBedrooms: 1,
            preferredSizeMin: 30,
            preferredSizeMax: 45,
            preferredFloorMin: 15,
            preferredFloorMax: null as number | null,
            preferredFacilities: ['Pool', 'Gym', 'Co-working Space'],
            hasPet: false,
            hasEvCar: false,
            parkingSlotsNeeded: 1,
            painPoints: 'Currently renting and paying 20K/month. Wants to build equity.',
            specialRequirements: 'Must have good view. Prefers north-facing units.',
            timeline: Timeline.ONE_TO_THREE_MONTHS,
            purposeOfPurchase: PurchasePurpose.OWN_USE,
            financingMethod: FinancingMethod.MORTGAGE,
            preApprovedAmount: 5000000,
            preApprovalExpiryDate: new Date('2026-06-30'),
        },
        {
            id: 'dddddddd-dddd-4ddd-8ddd-222222222222',
            workspaceId: bkkRealty.id,
            dealName: 'Sarah Miller — Investment Condo — Buy',
            dealType: DealType.BUY_SIDE,
            buyerContactId: buyerContacts[1].id,
            ...
            createdById: users[0].id,
            notes: 'Expat buyer. Interested in Ashton Asoke 2BR park view unit. Currently in negotiation on price.',
            budgetMin: 12000000,
            budgetMax: 16000000,
            preferredZoneIds: [zones[10].id, zones[4].id], // Asoke, Thonglor-Ekkamai
            preferredPropertyType: [PropertyType.CONDO],
            preferredBedrooms: 2,
            preferredSizeMin: 55,
            preferredSizeMax: 80,
            preferredFloorMin: 30,
            preferredFloorMax: null as number | null,
            preferredFacilities: ['Pool', 'Gym', 'Sky Lounge'],
            hasPet: true,
            hasEvCar: false,
            parkingSlotsNeeded: 1,
            painPoints: 'Wants park view. Previous condo had noisy neighbors.',
            specialRequirements: 'Must allow pets. Needs south-facing for park view.',
            timeline: Timeline.IMMEDIATE,
            purposeOfPurchase: PurchasePurpose.INVESTMENT,
            financingMethod: FinancingMethod.CASH,
            preApprovedAmount: null as number | null,
            preApprovalExpiryDate: null as Date | null,
        },
        {
            id: 'dddddddd-dddd-4ddd-8ddd-333333333333',
            workspaceId: bkkRealty.id,
            dealName: 'Pimchanok — Family Home — Buy',
            dealType: DealType.BUY_SIDE,
            buyerContactId: buyerContacts[2].id,
            ...
            assignedToId: users[2].id,
            createdById: users[1].id,
            notes: 'Referred by Tanawat. Young family with 2 kids, needs 3BR townhouse or house.',
            budgetMin: 8000000,
            budgetMax: 15000000,
            preferredZoneIds: [zones[0].id, zones[8].id, zones[9].id], // Sukhumvit, On Nut, Phra Khanong
            preferredPropertyType: [PropertyType.TOWNHOUSE, PropertyType.HOUSE],
            preferredBedrooms: 3,
            preferredSizeMin: 150,
            preferredSizeMax: 250,
            preferredFloorMin: null as number | null,
            preferredFloorMax: null as number | null,
            preferredFacilities: ['Garden', 'Parking'],
            hasPet: true,
            hasEvCar: true,
            parkingSlotsNeeded: 2,
            painPoints: 'Current condo too small for growing family. Needs more space and outdoor area for kids.',
            specialRequirements: 'Must be near international school. EV charger compatible parking.',
            timeline: Timeline.THREE_TO_SIX_MONTHS,
            purposeOfPurchase: PurchasePurpose.OWN_USE,
            financingMethod: FinancingMethod.MIXED,
            preApprovedAmount: 10000000,
            preApprovalExpiryDate: new Date('2026-09-15'),
        },
        {
            id: 'dddddddd-dddd-4ddd-8ddd-444444444444',
            workspaceId: bkkRealty.id,
            dealName: 'Kenji Tanaka — Ari Area — Buy',
            dealType: DealType.BUY_SIDE,
            buyerContactId: buyerContacts[3].id,
            listingId: null,
            ...
            createdById: users[0].id,
            notes: 'Japanese expat, working in Bangkok for 3 years. Wants investment property near Ari.',
            budgetMin: 4000000,
            budgetMax: 6000000,
            preferredZoneIds: [zones[3].id, zones[13].id], // Ari-Phahonyothin, Phaya Thai
            preferredPropertyType: [PropertyType.CONDO],
            preferredBedrooms: 1,
            preferredSizeMin: 28,
            preferredSizeMax: 40,
            preferredFloorMin: 10,
            preferredFloorMax: 30,
            preferredFacilities: ['Pool', 'Gym'],
            hasPet: false,
            hasEvCar: false,
            parkingSlotsNeeded: 0,
            painPoints: null as string | null,
            specialRequirements: 'Good rental yield is priority. Near BTS preferred.',
            timeline: Timeline.SIX_PLUS_MONTHS,
            purposeOfPurchase: PurchasePurpose.INVESTMENT,
            financingMethod: FinancingMethod.CASH,
            preApprovedAmount: null as number | null,
            preApprovalExpiryDate: null as Date | null,
        ...
        {
            id: 'dddddddd-dddd-4ddd-8ddd-555555555555',
            workspaceId: bkkRealty.id,
            dealName: 'Waraporn — Studio — Buy',
            dealType: DealType.BUY_SIDE,
            buyerContactId: buyerContacts[4].id,
            listingId: null,
            ...
            assignedToId: users[3].id,
            createdById: users[1].id,
            notes: 'Cold lead. Expressed mild interest in studio condos but has not responded to follow-ups.',
            budgetMin: 2500000,
            budgetMax: 4000000,
            preferredZoneIds: [zones[5].id, zones[7].id], // Rama 9, Ladprao
            preferredPropertyType: [PropertyType.CONDO],
            preferredBedrooms: 0,
            preferredSizeMin: 22,
            preferredSizeMax: 30,
    ...
    for (const deal of deals) {
        await prisma.deal.upsert({
            ...
            create: {
                ...
                // Buyer requirements (only for buy-side deals)
                budgetMin: (deal as any).budgetMin ?? null,
                budgetMax: (deal as any).budgetMax ?? null,
                preferredZoneIds: (deal as any).preferredZoneIds ?? [],
                preferredPropertyType: (deal as any).preferredPropertyType ?? [],
                preferredBedrooms: (deal as any).preferredBedrooms ?? null,
                preferredSizeMin: (deal as any).preferredSizeMin ?? null,
                preferredSizeMax: (deal as any).preferredSizeMax ?? null,
                preferredFloorMin: (deal as any).preferredFloorMin ?? null,
                preferredFloorMax: (deal as any).preferredFloorMax ?? null,
                preferredFacilities: (deal as any).preferredFacilities ?? [],
                hasPet: (deal as any).hasPet ?? null,
                hasEvCar: (deal as any).hasEvCar ?? null,
                parkingSlotsNeeded: (deal as any).parkingSlotsNeeded ?? null,
                painPoints: (deal as any).painPoints ?? null,
                specialRequirements: (deal as any).specialRequirements ?? null,
                timeline: (deal as any).timeline ?? null,
                purposeOfPurchase: (deal as any).purposeOfPurchase ?? null,
                financingMethod: (deal as any).financingMethod ?? null,
                preApprovedAmount: (deal as any).preApprovedAmount ?? null,
                preApprovalExpiryDate: (deal as any).preApprovalExpiryDate ?? null,
            },
        })
    }
